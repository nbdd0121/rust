        ...
        let block_and = 'block: {
            match expr.kind {
                ...
                ExprKind::LogicalOp { op, lhs, rhs } => {
                    ...
                    let rhs =
                        unpack!(else_block = this.as_local_operand(else_block, &this.thir[rhs]));
                ...
                ExprKind::Call { ty, fun, ref args, from_hir_call, fn_span } => {
                    if let ty::FnDef(def, _) = *ty.kind() {
                        if this.tcx.lang_items().box_new() == Some(def) {
                            let value = &this.thir[args[0]];
                            ...
                            let result = this
                                .local_decls
                                .push(LocalDecl::new(expr.ty, expr_span).internal());
                            this.cfg.push(
                                block,
                ...
                ExprKind::Use { source } => {
                    this.expr_into_dest(destination, block, &this.thir[source])
                }
                ...
                ExprKind::Adt(box Adt {
                    ...
                    let active_field_index =
                        if is_union { Some(fields[0].name.index()) } else { None };
                    ...
                    let operands = operands
                        ...
                        .map(|op| match *op {
                            thir::InlineAsmOperand::In { reg, expr } => mir::InlineAsmOperand::In {
                                reg,
                                value: unpack!(
                                    block = this.as_local_operand(block, &this.thir[expr])
                                ),
                            },
                ...
                ExprKind::Yield { value } => {
                    let scope = this.local_scope();
                    let value =
                        unpack!(block = this.as_operand(block, Some(scope), &this.thir[value]));
